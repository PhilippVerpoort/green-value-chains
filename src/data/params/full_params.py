...
def getFullParamsAll(basicData: dict, units: dict, times: list):
    r = []

    for category, item in basicData.items():
        df = __getFullParams(item, units, times)
        df['type'] = category
        r.append(df)
...
def __getFullParams(basicData: dict, units: dict, times: list):
    ...
    for parId, par in basicData.items():
        newPars = __calcValues(parId, par['value'], times)
        for newPar in newPars:
            if 'unit' in par and par['unit'] is not None:
                ...
                newPar['unit'] = newUnit
                newPar['techs'] = par['techs'] if 'techs' in par else 'bf,ng,h2'
            pars.append(newPar)

    return pd.DataFrame.from_records(pars, columns=['type', 'name', 'year', 'unit', 'value', 'uncertainty', 'uncertainty_lower', 'techs'])\
                    .rename(columns={'value': 'val', 'uncertainty': 'uu', 'uncertainty_lower': 'ul'})


# Iteratively calculate values for 1) keys (gwp100/gwp20, etc) and 2) different times (2025, 2050, etc).
def __calcValues(id:str, value: Union[dict, str, float], times: list):
    rs = []

    if isinstance(value, dict) and isinstance(list(value.keys())[0], str):
        for key, val in value.items():
            rs.extend(__calcValues(id+'_'+key, val, times))
        return rs
    ...
    elif not isinstance(list(value.keys())[0], str):
